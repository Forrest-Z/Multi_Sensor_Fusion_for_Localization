#include "../include/euclidean_cluster_core.h"

EuClusterCore::EuClusterCore(ros::NodeHandle &nh)
{
    seg_distance_ = {15, 30, 45, 60};
    cluster_distance_ = {0.5, 1.0, 1.5, 2.0, 2.5};
    sub_point_cloud_ = nh.subscribe("/kitti/velo/pointcloud", 5, &EuClusterCore::point_cb, this);
 ...
 * 
 * @param in_pc 
 * @param in_max_cluster_distance 聚类距离，不同圈子的聚类距离不同
 * @param obj_list 
 */
                                    ...
                                    std::vector<Detected_Obj> &obj_list)
{
    ...
    euclid.extract(local_indices); // 点云聚类的结果
