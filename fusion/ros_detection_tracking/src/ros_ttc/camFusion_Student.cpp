...
void removeOutliersCam(std::vector<std::pair<int, double>> &distMap)
{
    ...
    std::sort(distMap.begin(), distMap.end(), [](const std::pair<int, double> &a, const std::pair<int, double> &b) -> bool
    ...
    // 这套lambda表达式没看明白
    distMap.erase(std::remove_if(distMap.begin(), distMap.end(), [lower_bound, upper_bound](const std::pair<int, double> &ele)
                                 { return (ele.second < lower_bound || ele.second > upper_bound); }),
                  distMap.end());
}
