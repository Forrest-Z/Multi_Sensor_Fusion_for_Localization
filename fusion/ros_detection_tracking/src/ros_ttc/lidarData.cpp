...
#include "../../include/ros_detection_tracking/lidarData.hpp"


using namespace std;

// remove Lidar points based on min. and max distance in X, Y and Z
void cropLidarPoints(pcl::PointCloud<pcl::PointXYZ>::Ptr lidarPoints, float minX, float maxX, float maxY, float minZ, float maxZ)
{
    pcl::PointCloud<pcl::PointXYZ>::Ptr newLidarPts(new pcl::PointCloud<pcl::PointXYZ>); 
    for(pcl::PointCloud<pcl::PointXYZ>::const_iterator it =lidarPoints->points.begin(); it!=lidarPoints->points.end(); ++it) {
        
       if( it->x>=minX && it->x<=maxX && it->z>=minZ && it->z<=maxZ  && abs(it->y)<=maxY)  // Check if Lidar point is outside of boundaries
       {
           newLidarPts->points.push_back(*it);
       }
    }
...
//     float *data = (float*)malloc(num*sizeof(float)); //malloc 分配内存以字节（byte）为单位，并返回指向相应空间的指针，sizeof返回值也为字节单位
